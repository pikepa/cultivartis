            ...
            <div class="space-y-4">
                <div class='grid grid-cols-2 gap-4 mt-6'>
                    <div class="col-start-1">
                        ...
                        <input {{ $read ? '' : 'disabled' }} wire:model='first_name' type="text" name="first_name" id="first_name" autocomplete="first-name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <div class="col-start-2">
                        ...
                        <input wire:model='family_name' {{ $read ? '' : 'disabled' }} type="text" name="family_name" id="family_name" autocomplete="family-name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        @error('family_name') <span class="mt-1 text-sm text-red-500"> {{ $message }}</span> @enderror
                    </div>

                    <div class="col-start-1 col-end-2">
                        <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input wire:model='email' {{ $read ? '' : 'disabled' }} type="email" name="email" id="email" autocomplete="family-name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        @error('email') <span class="mt-1 text-sm text-red-500"> {{ $message }}</span> @enderror
                    </div>

                    <div class="col-span-2 col-start-1">
                        <label for="subject" class="block text-sm font-medium text-gray-700">Subject</label>
                        <input wire:model='subject' {{ $read ? '' : 'disabled' }} type="text" name="subject" id="subject" autocomplete="family-name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <div class="col-span-2 mt-1 sm:mt-0">
                        ...
                        <textarea wire:model='message_body' {{ $read ? '' : 'disabled' }} id="message_body" name="message_body" rows="5" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                    ...
                    <div class="col-span-2 mx-auto mt-6">
                        <span class="rounded-md shadow-sm ">
                            <button type="submit" {{ $read ? '' : 'disabled' }} class="flex justify-center w-full px-4 py-2 text-sm font-medium text-gray-700 border border-transparent rounded-md bg-gold hover:bg-yellow-200 focus:outline-none focus:border-indigo-700">
                                Send your Message
                            </button>
                        </span>
                    </div>
